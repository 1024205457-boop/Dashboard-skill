...
├── Step 3: Async report tasks (submit + poll + download CSV)
...
3. **Parallel without dependencies**: 5 fetch scripts are independent and run in parallel. Async report submission doesn't block other collection.
4. **Verify before writing**: Step 1 validates previous week's data integrity; aborts on anomalies.
5. **Minimal write scope**: The 4-week script only writes new data for the current week; historical weeks retain existing Excel values.
...
|-----------|-------------|--------------|
| Give formulas, not conclusions | Tell AI "conversion rate = conversion_uv / source_uv" | Don't say "about 4%~6%" |
| Give boundaries, not freedom | "col14=conversion/source_A, col15=conversion/source_B, col16=conversion/source_C" | Don't say "calculate the conversion rates" |
| Give validation, not trust | "Read back after writing to verify values match" | Don't say "just write it" |
| Give sources, not guesses | "metric_X = BI query (tag=filter_A + filter_B)" | Don't say "find the data yourself" |
...
|------|---------|------------|
| Read existing Excel data | Yes | |
| Compare against JSON source data | Yes | |
| Detect numerical discrepancies | Yes | |
| Judge whether discrepancy is acceptable (e.g., observation period not yet complete) | | Yes |
| Decide whether to overwrite and correct | | Yes |

### Step 2: Collect Current Week Data

| Task | AI does | Human does |
|------|---------|------------|
| Execute fetch scripts | Yes | |
| Check returned data is non-empty | Yes | |
| Detect cookie expiry and trigger refresh | Yes (runs refresh script) | Yes (scans QR code) |
| Judge if data magnitude is reasonable | Yes (compare to last week) | Yes (final confirmation) |
| Judge why data dropped | Hypothesize | Yes (confirm the reason) |

### Step 3: Async Report Tasks

| Task | AI does | Human does |
|------|---------|------------|
| Submit SQL report tasks | Yes | |
| Poll until completion | Yes | |
| Validate parameters match (prevent downloading stale records) | Yes | |
| Parse CSV (filter for target observation period rows) | Yes | |
| Judge whether SQL logic is correct | | Yes |

### Step 4-6: Data Writing

| Task | AI does | Human does |
|------|---------|------------|
| Calculate metrics by formula | Yes | |
| Write to specified Excel cells | Yes | |
| Fix historical formulas after insert_cols | Yes | |
| Confirm whether to write incomplete data | | Yes |
| Confirm whether to write anomalous values (e.g., -80% WoW) | | Yes |
...
**Lesson learned**: Once wrote incomplete mid-week data directly. Key metric showed 1,264 (actual was 3,930). All metrics were severely low.

### 2. CSV Must Filter by Observation Period
...
**Lesson learned**: Without filtering, user counts are 3x overcounted.

### 3. Polling Must Validate Parameter Match
...
**Lesson learned**: Latest query may return old successful records (different date range), causing missing data for certain weeks.
...
**Lesson learned**: Without fixing, "this week and last week have the same rate."

### 5. Formula Denominators Must Be Explicitly Specified

```python
# Different conversion rates have different denominators!
col14 = conversion_uv / source_A    # Source-A conversion rate
col15 = conversion_uv / source_B    # Source-B conversion rate
col16 = conversion_uv / source_C    # Source-C conversion rate
```

**Lesson learned**: All three columns used the same denominator, resulting in three identical rates.

### 6. Chart API Multi-Series Aggregation
...
- Each position in API selects represents a different filter dimension; must be specified by the human or captured from network requests
- SQL report template IDs and parameter formats must be exact — no "close enough"
- BI query template is large; field names, filter conditions, and date formats must strictly match the template
...
AI can report "This week's metric: 3,627; last week: 6,337; WoW: -42.8%", but cannot independently judge whether this is acceptable. Possible causes:
- Business operations changed (e.g., campaign stopped)
- Observation period incomplete (normal)
...
- 21-day observation period: weekEnd + 21 days before the conversion data is valid
- Some derived metrics use an "observation week" that is NOT the current week

### 6. Never Get Stuck at the Same Point More Than 3 Times

When the same error keeps recurring (e.g., version mismatch, API persistent 502), immediately inform the user and ask for help. Don't keep hammering.
...
Normal metrics:
  Metric A: 3,842 (last week 4,012, WoW -4.2%) [OK]
  Metric B: 1,523 (last week 1,491, WoW +2.1%) [OK]

Anomalous metrics (WoW change >30%):
  [!] Metric C: 3,627 (last week 6,337, WoW -42.8%)
  [!] Metric D: 297 (last week 1,194, WoW -75.1%)

Pending confirmation:
  - Weeks with incomplete 21-day observation: 0608, 0615 (low conversion is normal)
  - Metric E: BI returned null, retry succeeded, final value 523

Operations:
  [x] Cookie refresh
  [x] 5 fetch scripts completed
  [x] Async report tasks completed and downloaded
...
2. Script launches browser and opens login page
3. Human scans QR code (the only manual intervention point)
4. Script automatically saves all cookie files
...
| Metric Category | Data Source | Key Considerations |
|-----------------|-------------|-------------------|
| User acquisition / conversion | Internal analytics API (chart/detail) | Each position in selects has different meaning |
| Conversion funnel | Async SQL report (CSV) | Takes several minutes to complete |
| Channel metrics | Internal analytics API (chart) | Different stages have different denominators |
| Channel-specific conversion | SQL report CSV + BI query | CSV filter by observation period + exact filter match |
| CRM friend add/churn | CRM API | Specific group IDs required |
| Social media followers | Platform API | Requires platform-specific cookie + token |
| Channel-specific UV | BI query | Specific tag filters, NOT the overall total |
...
| Write to specified Excel positions   |  | Judge business reasons              |
| Detect WoW anomalies and report      |  | Decide whether to write incomplete  |
| Retry failed API calls               |  | Provide formulas for new metrics    |
| Validate parameter matching          |  | Final review of written results     |
...
3. **并行不依赖**：5个 fetch 脚本互不依赖，可并行执行。异步报表提交后不阻塞其他采集。
4. **先验证再写入**：Step 1 校验上周数据完整性，有异常则中止。
5. **最小化写入范围**：4周脚本只对当前周写入新数据，历史周保留 Excel 已有值。
...
| 原则 | 说明 | 反例 |
|------|------|------|
| 给公式不给结论 | 告诉 AI「转化率=转化uv/来源uv」 | 不要说「大约4%~6%」 |
| 给边界不给自由 | 「col14=转化/来源A, col15=转化/来源B, col16=转化/来源C」 | 不要说「算一下转化率」 |
| 给校验不给信任 | 「写入后读回来验证数值一致」 | 不要说「写完就行」 |
| 给来源不给推测 | 「指标X = BI查询(tag=筛选A+筛选B)」 | 不要说「数据你自己找」 |
...
| 执行 fetch 脚本 | 是 | |
| 检查返回数据非空 | 是 | |
| 判断 cookie 是否过期并刷新 | 是（执行刷新脚本）| 是（扫码） |
| 判断数据量级是否合理 | 是（与上周对比）| 是（最终确认） |
| 判断数据为什么下降 | 给出假设 | 是（确认原因） |
...
**教训**：曾把周中不完整数据直接写入，关键指标 1264（实际3930），所有指标严重偏低。
...
**教训**：最新查询可能返回旧的成功记录（范围不同），导致某周无数据。
...
**教训**：不修复会导致"这周和上周的比率一样"。

### 5. 公式分母必须明确指定

```python
# 不同转化率的分母不同！
col14 = 转化uv / 来源A    # 来源A转化率
col15 = 转化uv / 来源B    # 来源B转化率
col16 = 转化uv / 来源C    # 来源C转化率
```

**教训**：三列全用同一个分母，导致三个率完全相同。

### 6. Chart API 多 series 聚合
...
- API selects 每一位代表什么筛选，必须由人指定或从抓包获取
- SQL 报表模板 ID、参数格式必须精确，不能"差不多"
- BI 查询模板很大，字段名/过滤条件/日期格式必须与模板严格一致
...
AI 可以报告「本周指标C 3627，上周 6337，环比 -42.8%」，但不能自行判断这是否合理。可能原因：
- 业务操作变更（如活动停了）
- 观察期未满（正常）
- Cookie 过期导致采集为空（Bug）
...
- 本周数据：周一~周日（7天）
- 4周滚动：最近4个完整周
- 21日观察期：weekEnd + 21天后才是有效的转化数据
- 某些衍生指标的"观察周"≠当前周

### 6. 不能在同一位置卡住超过3次

遇到同一个错误反复出现（如版本不匹配、API 持续 502），立刻告知用户寻求帮助，不要死磕。
...
正常指标:
  指标A: 3,842 (上周 4,012, 环比 -4.2%) [OK]
  指标B: 1,523 (上周 1,491, 环比 +2.1%) [OK]

异常指标（环比变化 >30%）:
  [!] 指标C: 3,627 (上周 6,337, 环比 -42.8%)
  [!] 指标D: 297 (上周 1,194, 环比 -75.1%)

待确认:
  - 21日观察期未满的周: 0608, 0615（转化数据偏低属正常）
  - 指标E: BI返回null已重试成功，最终值 523
...
| 指标分类 | 数据源 | 关键注意点 |
|----------|--------|-----------|
| 用户获取/转化 | 内部分析平台 API (chart/detail) | selects 参数每位含义不同 |
| 转化漏斗 | 异步 SQL 报表 (CSV) | 需等几分钟完成 |
| 渠道指标 | 内部分析平台 API (chart) | 不同阶段有不同分母 |
| 渠道专项转化 | SQL报表 CSV + BI 查询 | CSV过滤观察期 + 精确筛选匹配 |
| CRM 好友新增/流失 | CRM API | 需要指定 group IDs |
| 社交媒体关注 | 平台 API | 需要平台特定 cookie + token |
| 渠道UV | BI 查询 | 特定标签筛选，不是总计 |
